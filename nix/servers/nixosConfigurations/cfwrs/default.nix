...
in
{
  ...
  services = {
    mysql = {
      ...
      ensureDatabases = [ "cfwrs" ];
      ensureUsers = [
        {
          name = "cfwrs";
          ensurePermissions = {
            "cfwrs.*" = "ALL PRIVILEGES";
          };
        }
        {
          name = "backup";
    ...
    phpfpm.pools.cfwrs = {
      ...
      phpOptions = ''
        auto_prepend_file = /srv/www/cfwrs/wp-content/plugins/crowdsec/inc/standalone-bounce.php
        expose_php off
        max_input_vars 2000
        ...
        { name, ... }:
        {
          ...
          extraConfig = # caddyfile
            ''
              ...
              encode zstd gzip
              root /srv/www/cfwrs
              php_fastcgi unix/${config.services.phpfpm.pools.cfwrs.socket} {
                # header_up Host cfwrs.org.uk
              }
              file_server
