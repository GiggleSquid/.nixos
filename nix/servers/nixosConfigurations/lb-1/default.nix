          ...
          "vrrp_notify_script" = nixpkgs.writers.writeBash "vrrp_notify_script.sh" ''
            ...
            case $ENDSTATE in
                "BACKUP") # Perform action for transition to BACKUP state
                   ${lib.getExe nixpkgs.ipvsadm} --start-daemon backup --syncid 1 --mcast-interface=$IFACE
                          exit 0
                          ;;
                ...
                "MASTER") # Perform action for transition to MASTER state
                   ${lib.getExe nixpkgs.ipvsadm} --start-daemon master --syncid 2 --mcast-interface=$IFACE
                          exit 0
                          ;;
